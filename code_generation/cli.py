...
def main(args):
    ...
    for image, image_filename in zip(images, sorted(os.listdir(args.image_directory))):
        ...
        base_name, ext = os.path.splitext(os.path.basename(image_filename))
        output_file = os.path.join(args.output_directory, f"{base_name}_output.html")
        
        if os.path.exists(output_file):
            print(f"Output file already exists for image: {image_filename}")
            continue
